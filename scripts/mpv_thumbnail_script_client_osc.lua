...
local thumbnailer_options = {
    ...
    autogenerate = true,
    height_boundary = 1080,
    fps_boundary = 60, -- adding boundary
    bit_depth_boundary = 9,
    avoid_hevc_autogenerate = false,
...
function Thumbnailer:register_client()
    ...
    mp.observe_property("video-dec-params", "native", function()
        ...
        if self.state.available and thumbnailer_options.autogenerate and check_pref() then -- modified to check preference
...
function display_thumbnail(pos, value, ass)
    ...
    if not ((duration == nil) or (value == nil)) then
        ...
        local text_h = 2 * msy -- 30 modified to 2
        local bg_h = ass_h + (display_progress and text_h or 0)
        local bg_left = pos.x - ass_w/2
        local framegraph_h = 2 * msy -- 10 modified to 2
        ...
        if user_opts.layout == "topbar" then
            ...
            text_top = bg_top
            framegraph_top = bg_top -- +20*msy modified to 0 (no sub bord)
        end
...
function check_pref()
    if (mp.get_property_number("height") or math.huge) > thumbnailer_options.height_boundary or (mp.get_property_number("container-fps") or math.huge) > thumbnailer_options.fps_boundary then
        return false
    elseif (mp.get_property_number("video-params/plane-depth") or 0) > thumbnailer_options.bit_depth_boundary then
        return false
    elseif mp.get_property("video-format") == "hevc" and thumbnailer_options.avoid_hevc_autogenerate then
        return false
    else
        return true
    end
